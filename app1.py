# app.py - Smart Hisaab Assistant with platform-aware mic handling
...
# --- Helper function for Speech-to-Text (local mic only) ---
def recognize_voice():
    """Microphone se voice ko recognize karke text mein badalta hai (local only)."""
    r = sr.Recognizer()
...
with col1:
    if platform.system() in ["Windows", "Darwin"]:  # Darwin = macOS
        ...
        st.info("🎙️ Voice input cloud/server par available nahi hai. Kripya text box use karein.")
        uploaded_file = st.file_uploader("Ya audio file upload karein", type=["wav", "mp3"])
        if uploaded_file:
            r = sr.Recognizer()
            with sr.AudioFile(uploaded_file) as source:
                ...
                try:
                    ...
                    st.success("Audio file se text extract ho gaya.")
                    st.rerun()
                except Exception as e:
                    st.error(f"Audio process karne mein error: {e}")
