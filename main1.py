...
def process_query_stream(api_key: str, user_story: str):
    """
    Streams the detailed LLM response chunk by chunk.
    """
    if not api_key:
    ...
    try:
        ...
        yield f"An error occurred while processing query: {e}"

def generate_audio_summary(api_key: str, detailed_text: str, slow: bool = True, lang: str = "hi"):
    """
    Generates a concise audio summary using gTTS.
    - slow: controls speech speed (True = slower, clearer)
